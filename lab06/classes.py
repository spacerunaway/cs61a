...
""" Implement Key here! """
class Key(Thing):
    def use(self, place,unlockplace):
        if unlockplace in place.exits:
            unlockplace = place.exits[unlockplace][0]
            if unlockplace.locked:
                unlockplace.locked = False
                print("{0} is now unlocked!".format(unlockplace.name))
            else:
                print("{0} is already unlocked!".format(unlockplace.name))

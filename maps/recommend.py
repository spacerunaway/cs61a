...
def find_predictor(user, restaurants, feature_fn):
    ...
    sxx,syy = mean([(xm - x)**2 for x in xs]), mean([(ym - y)**2 for y in ys])
    sxy = mean([(xm - x)*(ym - y) for x,y in zip(xs,ys)])
    b = sxy / sxx
    a, r_squared = ym - b * xm, sxy * sxy / (sxx * syy)
    # END Question 7
...
def rate_all(user, restaurants, feature_fns):
    ...
    for r in restaurants:
        ...
        if r in reviewed:
            d.update({r_name:user_rating(user, r_name)})
        else:
            d.update({r_name:predictor(r)})
    return d
    # END Question 9

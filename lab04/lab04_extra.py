...
def create_row(size):
    ...
    """
    "*** YOUR CODE HERE ***"
    return [base_board for i in range(size)]
...
def put_piece(board, max_rows, column, player):
    ...
    if max_rows == 0:
        return (row, board)
    elif is_empty(board[row][column]):
        replace_elem(board[row],column,player)
        return (row, board)
    else:
        return put_piece(board, row, column, player)

def is_empty(element):
    return element == base_board

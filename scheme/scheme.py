...
def scheme_eval(expr, env, tail=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 5     raise SchemeError('{0} is not scheme procedure'.format(first))
        "*** YOUR CODE HERE ***"
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError("{0} parameters need, but get {1} passed".format(len(formals),len(vals)))
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 6
        "*** YOUR CODE HERE ***"
        value = expressions.second.map(lambda operand:scheme_eval(operand,env)).first
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 15
    "*** YOUR CODE HERE ***"
    new_env = env.make_child_frame(nil, nil)
    while bindings is not nil:
        clause = bindings.first
        check_form(clause, 2, 2)
        key = clause.first
        val = clause.second.map(lambda operand:scheme_eval(operand,env)).first
        check_formals(Pair(key,nil))
        new_env.define(key,val)
    ...
    while True:
        try:
            ...
            while src.more_on_line:
                ...
                while (isinstance(result, Thunk)):
                    reslut = scheme_eval(result.expr, result.env)
